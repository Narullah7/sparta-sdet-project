class Planet
  ...
  def save
    ...
    if(self.id)
      sql = "UPDATE planet SET title='#{self.title}', body='#{self.body}', image='#{self.image}' WHERE id = #{self.id}"
    # If there Post has no id this means the post is new and needs creating
    else
      sql = "INSERT INTO planet (title, body, image) VALUES ('#{self.title}', '#{self.body}', '#{self.image}')"
    end
  ...
  def self.all
    ...
    sql = "SELECT id,title,body,image FROM planet ORDER BY id DESC"
